...
const UserJournal = () => {
	...
	useEffect(() => {
		...
		axios
			.get("https://localhost:7209/api/journal/getAll")
			// Filtering the response data to include only journals for the current user
			.then((response) => {
				const filteredJournals = response.data.filter(
					(journal) => journal.patient === userId
				);
			...
			<Container
						...
						<tbody>
							{journals.map((journal) => (
								<tr key={journal.journalnoteId}>
									...
									<td>
										<Button onClick={() => handleJournalClick(journal)}>
											Vis mer
										</Button>
									</td>
								</tr>
				...
				{selectedJournal && (
					...
					<Modal isOpen={modal} toggle={toggleModal}>
						...
						<ModalBody>{selectedJournal.journalnote}</ModalBody>
