...
function AdminEditUser () {
    ...
    const handleSearch = async (event) => {
      ...
      const handleUserSelect = async (user) => {
        ...
        try {
            const response = await fetch(`https://localhost:7209/api/user/getAll/${user.userId}`);
  ...
  <Table striped bordered hover>
    ...
    <tbody>
      {searchResults.map((user) => (
        <tr key={user.userId} onClick={() => handleUserSelect(user)}>
            <td><input type="checkbox" name="selectedUser" onChange={() => setSelectedUser(user)} checked={user === selectedUser}/></td>
    ...
    {selectedUser && (
    <Button variant="primary" onClick={() => setShowEditForm(true)}>Velg bruker</Button>
  )}
  </Table>
