...
const RegisterForm = () =>{
    ...
    const handleSubmit = (event) => {
        event.preventDefault();
         if (password !== confirmPassword || !/[a-åA-Å]/.test(password) || /[0-9]/.test(password) || password.length < 8){
            setErrorMessage ('Passordet må inneholde mist 8 karakterer, inkludert minst en bokstav og ett nummer');
            return;
        }
        ...
        fetch('api/checkUser', {
            ...
            .then(data => {
                ...
                setErrorMessage('');
                alert('Bruker registrert vellykket. Informasjon: ${JSON.stringify(data)}')
            })
    ...
    const handleZipcodeChange = (event) => {
        ...
        if(zipcode.length === 4){
            fetch('api/zipcode?code=${zipcode}')
                .then((response) => {
                    if(response.ok){
    ...
    return(
        <div className="d-flex justify-content-center align-items-start min-vh-100" style={{paddingTop: '0px', position:'relative'}}>
            ...
            <Form onSubmit={handleSubmit}>               
                ...
                <Card  style={{width: '500px', marginBottom: "50px" }} >
                    ...
                    <Card.Body>                
                        ...
                        <Button variant="primary" type="submit" style={{paddingLeft: '150px',paddingRight: '150px', marginTop: '30px', marginLeft:'35px', backgroundColor:'#0050B1'}}>
                            Register Bruker
                        </Button>
                        <Form.Group style={{marginTop: '30px'}}>  
                            ...
                            <Link to="/login" onClick={handleFlipCard} className="flipLink" style={{marginLeft:'50px'}}>Har du bruker? Klikk her! </Link>
                        </Form.Group>
                    </Card.Body>
                </Card>
            </Form>
